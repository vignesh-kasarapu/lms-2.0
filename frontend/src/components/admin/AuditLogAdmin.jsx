...
export default function AuditLogAdmin() {
  ...
  useEffect(() => { getAuditLog({}).then((res) => setRows(res.data)).finally(() => setLoading(false)); }, []);
            ...
            <div key={r.audit_id} className="py-2.5">
              <div className="flex items-center justify-between">
                <p className="text-sm text-slate-100">
                  {r.is_system_actor ? 'SYSTEM' : r.actor?.full_name || `#${r.actor_id}`} · <span className="text-slate-400">{r.action}</span>
                </p>
                <span className="text-xs text-slate-600">{new Date(r.timestamp).toLocaleString()}</span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">{r.entity_type} #{r.entity_id}</p>
            </div>
          ))}
        </div>
      )}
