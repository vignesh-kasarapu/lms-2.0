  ...
  return (
    <header className="glass-panel flex items-center justify-between px-6 py-4 mb-6 relative z-40">
      <div className="flex items-center gap-3.5">
        ...
        {isHrAdmin ? (
          <span className="role-badge-admin hidden md:inline-flex text-xs font-bold px-3 py-1 items-center gap-1.5 shadow-md shadow-indigo-500/20">
            <ShieldCheck className="w-4 h-4 text-indigo-300" /> HR Admin Portal
          </span>
        ) : isManager ? (
          <span className="role-badge-hr hidden md:inline-flex text-xs font-bold px-3 py-1 items-center gap-1.5 shadow-md shadow-emerald-500/20">
            <UserCheck className="w-4 h-4 text-emerald-300" /> Manager Portal
          </span>
        ) : (
          <span className="role-badge-employee hidden md:inline-flex text-xs font-bold px-3 py-1 items-center gap-1.5 shadow-md shadow-cyan-500/20">
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-3 pl-4 border-l border-white/15 hover:bg-white/5 p-1.5 rounded-2xl transition-all border border-transparent hover:border-white/10 group focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
            title="Click to view full user profile & settings"
            aria-expanded={isProfileOpen}
          >
            <div className="relative">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-black text-white shadow-lg transition-transform group-hover:scale-105 ${
                isHrAdmin
              ...
              <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-emerald-400 ring-2 ring-void-950 animate-pulse" />
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-sm font-bold text-white leading-none group-hover:text-emerald-300 transition-colors">
                {emp.full_name || 'User Profile'}
              </p>
              <p className="text-xs text-slate-300 font-semibold leading-none mt-1.5">
                {emp.designation || 'Staff'}
              </p>
            </div>
            <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isProfileOpen ? 'rotate-180 text-emerald-400' : 'group-hover:text-white'}`} />
          </button>

          {/* Profile Pop-Up Modal Card */}
          {isProfileOpen && (
            <div className="absolute right-0 mt-3 w-80 sm:w-96 rounded-3xl glass-panel p-5 shadow-2xl border border-white/20 z-50 animate-in fade-in slide-in-from-top-3 duration-200 backdrop-blur-2xl bg-slate-950/90 text-slate-100">
              ...
              <div className="flex items-start justify-between pb-4 border-b border-white/10">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    ...
                    <span className="inline-block text-[10px] font-mono px-2 py-0.5 mt-1 rounded bg-white/10 text-slate-300 border border-white/10">
              ...
              <div className="py-3 border-b border-white/10">
                <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">Assigned System Roles</p>
              ...
              <div className="py-3 space-y-2.5 border-b border-white/10 text-xs">
                {emp.email && (
                ...
                {emp.supervisor && (
                  <div className="flex items-center gap-2 text-slate-300">
                    ...
                    <span>Reports to: <strong className="text-slate-100">{emp.supervisor.full_name}</strong></span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-slate-300">
      ...
      {showLogoutConfirm &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
            <div className="glass-panel p-6 rounded-3xl max-w-md w-full border border-red-500/30 bg-slate-950/95 text-slate-100 shadow-2xl space-y-4">
              <div className="flex items-center gap-3.5 text-red-400">
                ...
                <div>
                  <h3 className="font-display font-extrabold text-lg text-white">Sign Out Confirmation</h3>
                  <p className="text-xs text-slate-300">End active session & logout of workspace</p>
                </div>
              </div>
