...
export default function Table({ columns, children, maxHeight = 'max-h-[70vh]', className = '' }) {
  return (
    <div className={`overflow-auto ${maxHeight} rounded-2xl border border-frost/10 ${className}`}>
      <table className="w-full min-w-[640px] text-sm border-collapse">
        <thead className="sticky top-0 z-10">
          <tr className="bg-ink-950/95 backdrop-blur-xl border-b border-frost/10">
            {columns.map((col) => (
              <th
                key={col.key || col.label}
                className={`text-left px-4 py-3 text-[11px] font-bold uppercase tracking-wider text-ink-400 whitespace-nowrap ${col.className || ''}`}
              >
                {col.label}
              </th>
            ))}
        ...
        <tbody className="divide-y divide-frost/5">{children}</tbody>
      </table>
    </div>
  );
}
